...
def plot_combined_scores(n_clusters, scores, filename):
    ...
    plt.savefig(f"Images/dr_{filename}_combined_scores.png")
    plt.close()
...
def get_best_k_table(n_clusters, scores_list, dataset_name):
    ...
    table.scale(1.1, 1.3)
    plt.savefig(f"Images/dr_{dataset_name}_em_summary_table.png")
    plt.close()

    return result_df

def run_em_on_reduced(X_reduced, y, filename):
    n_clusters = list(range(2, 10))
    scores = []
    for k in n_clusters:
        gmm = GaussianMixture(n_components=k, covariance_type='full', n_init=1, max_iter=100, random_state=42)
        scores.append(bench_em(gmm, y, X_reduced))
...
def run_all_dr_em(file_path, dataset_name):
    ...
    if dataset_name == "cancer":
        ...
        dr_params = {"pca": 6, "ica": 6, "rp": 4}
    elif dataset_name == "bankruptcy":
        label_col = "Bankrupt?"
        dr_params = {"pca": 53, "ica": 8, "rp": 30}
    else:
    ...
    for method, model in zip(["pca", "ica", "rp"], [PCA, FastICA, GRP]):
        X_red = model(n_components=dr_params[method], random_state=42).fit_transform(X_scaled)
        run_em_on_reduced(X_red, y, f"em_{method}_{dataset_name}")

if __name__ == "__main__":
    run_all_dr_em("data/bankruptcy_data.csv", "bankruptcy")
    run_all_dr_em("data/global_cancer_patients_2015_2024.csv", "cancer")
