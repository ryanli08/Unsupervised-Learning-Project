...
def plot_combined_scores(n_clusters, scores, filename):
    ...
    plt.savefig(f"Images/{filename}_kmeans_combined_scores.png")
    plt.close()
...
def get_best_k_table(n_clusters, scores_list, dataset_name):
    ...
    table.scale(1.1, 1.3)
    plt.savefig(f"Images/kmeans_{dataset_name}_summary_table.png")
    plt.close()

    return result_df

def run_kmeans(file_path, dataset_name):
    ...
    for k in n_clusters:
        ...
        scores.append(bench_kmeans(kmeans, y, X_scaled))

    plot_combined_scores(n_clusters, scores, f"kmeans_{dataset_name}")
    summary_table = get_best_k_table(n_clusters, scores, dataset_name)
    print(f"\n=== Summary for {dataset_name} ===")
    print(summary_table)
    return summary_table

if __name__ == "__main__":
    run_kmeans("data/bankruptcy_data.csv", "bankruptcy")
    run_kmeans("data/global_cancer_patients_2015_2024.csv", "cancer")
